...
function runExample() {
  ...
  const steps = [
    ...
    },
    {
      ...
      /*
       * 이 화면에는 모델이 둘 지납니다. 이 칸을 채우는 문장 모델은 부르기만 하고, 다음
       * 단계의 그림 모델은 우리가 띄웁니다. 그 갈림을 여기서 한 번 적어 두고, 다음
       * 단계에서 「우리가 띄우는 쪽」의 값을 적습니다(onboard.js 의 note).
       */
      note: {
        head: '이 칸도 부르기만 합니다',
    ...
    },
    {
      ...
      note: {
        head: '직접 들 것만 직접 듭니다',
        body: '문장 모델은 부르기만 하고, *그림 모델만 우리가 띄웁니다*. 가중치가 공개된 모델이라 '
          + '그림 결이 우리 것이 되고, 대본이 이 계정 밖으로 나가지 않습니다.',
      },
      run: () => {
        S.step = 3
        paint()
...
const KV_CARDS = [
  ...
  },
  {
    ...
    spot: ['tab2', 'rig'],
    tags: [{ on: 'tab2', text: '문장 모델 · 부릅니다' }, { on: 'rig', text: '그림 모델 · 우리가 띄웁니다' }],
    next: '다음', skip: '건너뛰기',
  },
  {
