 ...
 * 안내 자체는 각 화면이 이미 들고 있고(runExample), 화면을 잇는 것은 demo.js 가 합니다 —
 * 어느 자리를 짚어야 하는지는 그 화면만 알기 때문에 여기로 베껴 오지 않습니다.
 */
function wireDemo() {
  const b = byId('demoGo')
 ...
 * 자르면 「이게 전부」로 읽힙니다.
 *
 * 판별 GSI 를 하나 세워 한 번에 읽는 길도 있습니다. 지금은 세우지 않았습니다 —
 * 홈이 한 번 읽어 그리면 끝이고, 카드 열 장 남짓의 팀에서는 이 편이 훨씬 단순합니다.
...
async function loadActs() {
  ...
  /*
   * 판마다 따로 줄로 옮긴 뒤 합칩니다. 한꺼번에 옮기면 어느 판의 일인지가 사라집니다 —
   * op 자체에는 boardId 가 없습니다(pk 에만 있습니다). 덩어리로 묶는 것도 판 안에서만
