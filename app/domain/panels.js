 ...
 * 배경 한 장, 소품 한 장을 klein 으로 따로 그려 내어 자산으로 둡니다(pages/board.js 의
 * extractAssets). 다음 컷은 그 자산들을 참조 이미지로 여러 장 받아 새 구도를 그립니다.
 *
 * 여기는 「무엇을 뽑을지」만 정합니다. 그림을 부르는 것은 화면이고, 저장은 op(asset.add)입니다.
 */
export const ASSET_TYPES = { char: '인물', bg: '배경', prop: '소품', still: '스틸' }
/** 참조로 넣을 수 있는 종류. 스틸은 자산들로 만든 결과물이라 다시 참조로 넣지 않습니다 */
export const REF_TYPES = ['char', 'bg', 'prop']

/**
 * 승인된 컷 하나에서 뽑을 자산 목록. 인물은 컷에 붙인 등장 인물마다 하나, 배경·소품은 하나씩.
 *
...
export function assetJobs(panel, chars = {}) {
  ...
  jobs.push({
    type: 'prop', name: where ? `${where} 소품` : '소품', kind: 'asset', refKind: 'asset_prop',
