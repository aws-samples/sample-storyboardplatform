...
- Screens: [`app-walkthrough/screens/`](app-walkthrough/screens/)
...
everything if it broke. The prompts live in the browser (`app/story.js`) so changing the wording
...
- Ops from other people must pass `scrub()` in `app/core.js` before they touch state. That is the trust boundary.
...
app/                 browser. no framework, no build. this directory becomes the bucket root
  ...
  board.js             every screen and interaction: board, detail, viewer, markup, timeline, print
  story-graph.html     story develop + script-to-graph screen, all in one file
  key-visual.html      key visual: script → one image per scene
  key-visual.js        that screen's logic · imports the shared modules below
  nav-tabs.js          the top tab bar the four screens share, plus NAV_TABS (the step order)
  theme.css            the one set of colours and fonts (--sb-*) all four screens read
  coach.js             onboarding coachmarks · the veil, the bubble, the keyboard handling
  onboard.js           the guided example: which card points at what, in which order
  ...
  net.js               transport. AppSync when configured, BroadcastChannel between tabs when not.
                       board.js cannot tell which one it got
...
app-walkthrough/     example only. no product code imports anything under here
  ...
  screens/             screenshots and the key-visual mockup
infra/
...
`app/seed-art.js` ships empty, so the starter board draws canvas placeholders · the same locally and
...
> the scripts can opt out of. If you are done with the demo for more than a week, either re-run
> `stop.sh` each week or `npx cdk destroy` · otherwise the cluster quietly comes back and bills.
...
- Story planning is the one call with no retry and no outbox. A paid call must not be re-sent behind
  ...
  off (`retryAttempts: 0`) for the same reason.
- A plan job outlives the page. Close the tab mid-plan and the Lambda still finishes and pays for
  the tokens; nothing reads the result and the TTL sweeps it an hour later.
