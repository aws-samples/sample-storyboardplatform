 ...
 *
 * 원래 key-visual/coach.js 였고 카드 내용이 이 파일에 박혀 있었다. 세 화면이 다 쓰게
 * 되면서 demo/ 로 옮기고(배포에서 demo/ 만 세 위치에 다 올라간다) 카드는 화면이
 * 넘겨주는 것으로 바꿨다. 엔진은 좌표·막·말풍선·키보드만 맡는다.
 *
 * 어느 화면에서든 지켜야 하는 것 — 카드는 지금 화면에 실제로 있는 것만 가리킨다.
 * 앵커가 없으면 그 장은 조용히 건너뛴다. 빈 구멍을 가리키면 코치마크가 먼저 신뢰를
 * 잃는다. 같은 이유로 여기에 쓰지 않는 말이 있다 — 빠르다 · 무한히 확장된다 ·
 * 제작 기간이 줄어든다. 화면에서는 8장이 몇 분 걸리고 한 장은 실패한다.
 *
 * CSS 도 이 파일이 넣는다(nav-tabs.js 와 같은 방식). 화면마다 40줄씩 베껴 두면
 * 한 곳만 고쳐도 나머지가 어긋난다.
 */
...
/*
 * 막 위의 층. 여기서 쓰는 색은 공통 토큰(demo/theme.css)에서 받고, 두 번째 인자는
 * 그 파일이 없을 때의 기본값이다.
 */
const CSS = `
