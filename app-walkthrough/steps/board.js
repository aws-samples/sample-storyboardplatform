...
function runExample() {
  ...
  /*
   * 자산. 승인이 만드는 두 번째 것입니다. 왼쪽 기둥의 「자산」 머리글을 짚습니다 — 목록은
   * 자산이 있어야 서고, 예시 판에는 없습니다. 생성 서버가 없는 배포에는 칸 자체가 숨어
   * 있어(board.js 의 renderAssets) 그때는 테 없이 말만 띄웁니다.
   */
  steps.push({
    say: '승인하면 자산이 됩니다',
    sub: '승인된 그림에서 인물·배경·상품을 따로 그려 왼쪽 「자산」에 모아 둡니다. 다음 컷을 '
      + '만들 때 「참조 자산」으로 골라 넣으면 같은 얼굴·같은 장소·같은 물건으로 새 구도가 나옵니다. '
      + '컷에 붙인 인물의 자산과 같은 씬의 배경은 저절로 들어갑니다.',
    spot: '#assetHead',
    see: 'board',
    run: () => showCuts(),
  })
