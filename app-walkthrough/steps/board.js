 ...
 *
 * 첫 걸음과 셋째 걸음은 시나리오 칸(#scenario)과 「컷으로 분해」(#breakdown)를 짚었습니다.
 * 둘 다 「시나리오를 컷으로」 창 안으로 들어갔고(board.html 의 #cuts), 닫힌 dialog 안은
 * 보이지 않아서 테를 걸 수 없습니다 — 그 창을 여는 단추를 대신 짚습니다. 두 걸음이 같은
 * 자리를 짚지만, 실제로 그 단추 하나가 「시나리오를 적고 컷으로 나누는」 자리입니다.
 */
const EXAMPLE_SPOTS = ['cuts', '#newChar', 'cuts', 'board']
 ...
 * 인물의 판으로 넘어가는데(board.js 의 pickView 는 아무것도 안 골려 있으면 첫 인물을
 * 고릅니다), 인물 판에서는 「시나리오를 컷으로」 단추가 숨습니다(renderNav). 다음 걸음이
 * 짚을 자리가 그것이라 테가 사라졌습니다.
 *
 * 화면이 어긋나는 것이 더 큰 문제였습니다. 셋째 걸음은 3초를 기다린 뒤 「컷 N개가
