...
export function request(ctx) {
  ...
  // 결과를 읽을 수 있는 사람을 여기서 못 박는다. planResult 가 같은 값으로 대조한다. // jobId 를 주워도 남의 결과는 못 읽는다.
  const owner = claims['cognito:username'] || claims.sub || ''
  const jobId = util.autoId()
  ctx.stash.jobId = jobId

  // maxTokens·model·think 는 손대지 않고 넘긴다. 클램프와 허용 목록은 Lambda 쪽에 있다. // APPSYNC_JS 에는 Number() 도 없어서 검사를 두 군데 두면 규칙이 어긋나기 쉽다.
  return {
    operation: 'Invoke',
