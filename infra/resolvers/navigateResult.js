// navigate 잡 하나의 결과를 읽는다. Ops 테이블(OpsDs)에 앉는다.
// planResult.js 와 같은 항목을 읽는다 — Lambda 가 두 잡을 같은 키(PLAN#jobId)로 적는다.
//
// 브라우저가 이걸 폴링한다. 아직 안 끝났으면 항목이 없고,
// 그때는 오류가 아니라 {status:"pending"} 을 돌려준다 — 부르는 쪽이 계속 물어보게.
//
// Lambda 는 성공이든 실패든 반드시 항목을 적는다. 안 적으면 브라우저가 타임아웃까지 기다린다.
...
export function response(ctx) {
  ...
  // 잡을 띄운 사람과 지금 읽는 사람이 같은지 대조한다. 인자로 온 owner 는 믿지 않는다 —
  // 대조는 토큰의 클레임으로만 한다.
  const claims = ctx.identity?.claims || {}
